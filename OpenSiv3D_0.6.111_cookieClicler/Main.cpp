...
{
	// の絵文字
	const Texture texture{ U"❤️"_emoji };


	const Texture catEmoji{ U"🐈"_emoji };

	// 農場の絵文字
	const Texture tigerEmoji{ U"🐅"_emoji };

	// 工場の絵文字
	const Texture HyouEmoji{ U"🐆"_emoji };
	...
	while (System::Update())
	{
		...
		homeCost = 10 + (homeCount * 10);

		// 農場の価格を計算する
		farmCost = 50 + (farmCount * 50);
		...
		// 農場ボタン
		if (Button(Rect{ 340, 210, 420, 100 }, tigerEmoji, font, U"動物園", U"C{} / 1 CPS"_fmt(farmCost), farmCount, (farmCost <= cookies)))
		{
			cookies -= farmCost;
			++farmCount;
		}
		...
		// 工場ボタン
		if (Button(Rect{ 340, 350, 420, 100 }, HyouEmoji, font, U"サバンナ", U"C{} / 10 CPS"_fmt(factoryCost), factoryCount, (factoryCost <= cookies)))
		{
			cookies -= factoryCost;
			++factoryCount;
		}
